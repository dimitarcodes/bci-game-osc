...
phase_a.a4.desc =
  "You take a look at the sampling frequency encoded in the metadata of the files, it appears that the data was sampled at 250Hz.";
phase_a.a4.hint =
...
phase_a.a5.desc =
  "There are 22 EEG channels, sadly their naming scheme isn't very informative about the layout";
phase_a.a5.hint =
...
phase_b.available =
  " You've already done the most important steps of this phase. You can probably move on to the spatial filter selection phase - type 'go to spatial filtering phase'.";
phase_b.hint = "";
...
phase_b.b3.desc =
  "You tried removing noisy channels by using a min-max threshold criterion. No channels were removed from the data. There are already only 22 channels in the data, perhaps the noisy channels were already removed before publication.";
phase_b.b3.hint =
  "There are other steps in the preprocessing phase that are also required. Can we limit the number of samples for example? Do we have to split up the signals in some way?";
...
phase_b.b4.hint =
  "This was a step we wanted you to do, good job. It is however still important you do at least 1 other important step in the preprocessing phase before we can learn anything.";
...
phase_b.b5.desc =
  "You want to spectral filter the data, smart! What frequency band do you think is best in this scenario?: <br> <br> A: [1 - 10] Hz <br> B: [7-20] Hz <br> <br> type: 'choose band A' or 'choose band B' to continue ";
...
phase_b.b5A.desc =
  "You applied a band pass filter that limited the frequency contents to the [1-10] Hz band.";
phase_b.b5A.hint =
...
phase_b.b5B.desc =
  "You applied a band pass filter that limited the frequency contents to the [7-20] Hz band. Hopefully you did this because you know this is a motor imagery task and attempted to preserve content related to this task and didn't just guess the answer..";
phase_b.b5B.hint =
  "Did you already limit the number of samples to speed up processing later on?";
...
phase_b.b6.desc =
  "You cut the data into epochs of [-1, 7]s, such that each epoch contains the EEG-signals of one of the two classes. Remember the markers, these contain the classes we are interested in.";
phase_b.b6.hint =
  "This could be the last step of the preprocessing phase. You could try to 'go to choosing spatial filter'. If you're not allowed to do so, you probably missed a step.";
...
phase_c.c0.desc =
  "You've finished preprocessing the data. Now it's time to bring the data into a space such that we can perform some analysis or train a model.";
(phase_c.c0.hint =
  "Remember the markers, what can we do with this data? We want to bring the data into a space/shape such that we can learn something useful from this paradigm."),
...
phase_c.c1.desc =
  "You apply Principal Component Analysis and receive 22 components. What else can you do, what are we working towards?";
phase_c.c1.hint =
  "Remember the markers, what are the targets? Do we want to work towards classification or regression?";
phase_c.c1.dests = ["C2", "C3"];
phase_c.c1.unavailable =
  "Great that you're already thinking about PCA, but we'll keep that along with other spatial filtering methods for the next phase - 'spatial filter phase'. We'll tell you once you can move on to that phase ;)";
phase_c.c1.regex = /^(.*)(pca|principal component)(.*)$/;

phase_c.c2 = [];
phase_c.c2.alias = "ICA";
phase_c.c2.desc =
  "You apply Independent Component Analysis and receive 22 components. What else can you do, what are we working towards?";
phase_c.c2.hint =
  "Remember the markers, what are the targets? Do we want to work towards classification or regression?";
phase_c.c2.dests = ["C1", "C3"];
phase_c.c2.unavailable =
  "Great that you're already thinking about ICA, but we'll keep that along with other spatial filtering methods for the next phase - 'spatial filter phase'. We'll tell you once you can move on to that phase ;)";
...
phase_c.c3.hint =
  "What do the filters of CSP learn? What do the filters from both sides of the spectrum stand for? What information does the band-power of the CSP components have? Can you transform this information into features?";
...
phase_c.c4.desc =
  "Are you sure? What were the markers? What is SPoC used for? Is there a better method?";
...
phase_d.available =
  "You *could* already go to the next phase: 'model selection'. IF you wish to do that, type 'go to model selection'.";
phase_d.hint = "";

phase_d.d1A = [];
phase_d.d1A.alias = "Components with the largest eigenvalues";
phase_d.d1A.desc =
  "For each epoch, you reduce t he number of components to only 4, namely the ones with the largest corresponding eigenvalues. Can you transform these components into features for a model to train on?";
phase_d.d1A.hint =
  "What information does the power of the CSP components have? Can you transform this information into features?";
...
phase_d.d1B.desc =
  "For each epoch, you reduce the number components to only 4 in total, 2 from both sides of the eigenvalue spectrum. Can you transform these components into features for a model to train on?";
phase_d.d1B.hint =
  "What information does the power of the CSP components have? Can you transform this information into features?";
...
phase_d.d1C.desc =
  "For each epoch, you reduce t he number of components to only 4, namely the ones with the smallest corresponding eigenvalues. Can you transform these components into features for a model to train on?";
phase_d.d1C.hint =
  "What information does the power of the CSP components have? Can you transform this information into features?";
...
phase_d.d2.desc =
  "For each component, you compute the average amplitude. <br> You have now entered the 'model selection' phase as there was nothing left for you to do in this feature extraction phase.";
phase_d.d2.hint = "Why the average? What can we do with these values?";
phase_d.d2.dests = ["E0"];
phase_d.d2.regex =
  /^(.*)(average amplitude|average csp component|average component)(.*)$/;

phase_d.d3 = [];
phase_d.d3.alias = "Compute average log-band power";
phase_d.d3.desc =
  "For each component, you compute the average log band power. <br> You have now entered the 'model selection' phase as there was nothing left for you to do in this feature extraction phase.";
...
phase_d.d4.desc =
  "For each component, you compute the average envelope. <br> You have now entered the 'model selection' phase as there was nothing left for you to do in this feature extraction phase.";
...
phase_f.f2.desc =
  "You decide to evaluate your model using classification accuracy.";
phase_f.f2.dests = [];
...
misc.psd.desc = [
  "You plot the raw data and the raw data's frequency spectrum (PSD)",
...
createRoom("C1", {
  ...
  afterEnter: function () {
    setHint(phase_c.c1.hint, phase_c);
  },
  regex: phase_c.c1.regex,
  dests: phase_c.c1.dests
    .map((x) => new Exit(x))
    .concat(
      new Exit("C4", {
        simpleUse: function (char) {
...
createRoom("C2", {
  ...
  afterEnter: function () {
    setHint(phase_c.c2.hint, phase_c);
  },
  regex: phase_c.c2.regex,
  dests: phase_c.c2.dests
    .map((x) => new Exit(x))
    .concat(
      new Exit("C4", {
        simpleUse: function (char) {
  ...
  afterEnter: function () {
    ...
    if (phase_b.gate % 5 == 0) {
      if (phase_d.gate % 3 == 0) {
        if (phase_d.gate % 7 == 0) {
          if (phase_e.gate % 2 == 0) {
            if (phase_f.gate % 2 == 0) {
              ...
              msg(
                "You get accuracy of 57.14285714285714%! That's pretty low :/",
              );
            } else if (phase_f.gate % 5 == 0) {
              msg(
                "You get average cross-validation score of 50.02463054187192%! Yikes D:",
              );
            }
            ...
            if (phase_f.gate % 2 == 0) {
              ...
              msg(
                "You get average cross-validation score of 50.71428571428571%! Yikes D:",
              );
            }
            ...
            if (phase_f.gate % 2 == 0) {
              ...
              msg(
                "You get average cross-validation score of 50.04926108374385%! Yikes D:",
              );
            }
          }
          ...
          if (phase_e.gate % 2 == 0) {
            if (phase_f.gate % 2 == 0) {
              ...
              msg(
                "You get average cross-validation score of 85.36945812807882%! Flag: Br4inzG0Round&Round",
              );
            }
          } else if (phase_e.gate % 3 == 0) {
            if (phase_f.gate % 2 == 0) {
              ...
              msg("You get accuracy of 78.57142857142857%! You can do better!");
            } else if (phase_f.gate % 5 == 0) {
              msg(
                "You get average cross-validation score of 87.4384236453202%! Flag: Br4inzG0Round&Round",
              );
            }
          } else if (phase_e.gate % 5 == 0) {
            if (phase_f.gate % 2 == 0) {
              ...
              msg(
                "You get average cross-validation score of 87.41379310344828%! Flag: Br4inzG0Round&Round",
              );
            }
          }
          ...
          if (phase_e.gate % 2 == 0) {
            if (phase_f.gate % 2 == 0) {
              ...
              msg("You get accuracy of 71.42857142857143%! You can do better!");
            } else if (phase_f.gate % 5 == 0) {
              msg(
                "You get average cross-validation score of 81.92118226600984%! Flag: Br4inzG0Round&Round",
              );
            }
          } else if (phase_e.gate % 3 == 0) {
            if (phase_f.gate % 2 == 0) {
              ...
              msg(
                "You get average cross-validation score of 79.08866995073891%! Flag: Br4inzG0Round&Round",
              );
            }
          } else if (phase_e.gate % 5 == 0) {
            if (phase_f.gate % 2 == 0) {
        ...
        msg(
          "Yikes, you took the wrong CSP components and got chance-level performance :( Should've picked components from both ends of the eigenvalue spectrum",
        );
      }
    } else if (phase_b.gate % 3 == 0) {
      msg(
        "Yikes, you spectrally filtered the data to a band you don't care about. You obtained chance level performance, as the task at hand has information in the other band. Reload the page to try again, pay attention to the markers!",
      );
      ...
      if (phase_d.gate % 3 == 0) {
        if (phase_d.gate % 7 == 0) {
          if (phase_e.gate % 2 == 0) {
            if (phase_f.gate % 2 == 0) {
              ...
              msg(
                "You get average cross-validation score of 53.49753694581281%! Yikes D:",
              );
            }
          } else if (phase_e.gate % 3 == 0) {
            if (phase_f.gate % 2 == 0) {
              ...
              msg(
                "You get average cross-validation score of 0.49334975369458134%! Yikes D:",
              );
            }
          } else if (phase_e.gate % 5 == 0) {
            if (phase_f.gate % 2 == 0) {
              ...
              msg(
                "You get average cross-validation score of 51.40394088669951%! Yikes D:",
              );
            }
          }
        } else if (phase_d.gate % 11 == 0) {
          if (phase_e.gate % 2 == 0) {
            if (phase_f.gate % 2 == 0) {
              ...
              msg(
                "You get average cross-validation score of 62.63546798029557%!",
              );
            }
          } else if (phase_e.gate % 3 == 0) {
            if (phase_f.gate % 2 == 0) {
              ...
              msg(
                "You get average cross-validation score of 62.61083743842365%!",
              );
            }
          } else if (phase_e.gate % 5 == 0) {
            if (phase_f.gate % 2 == 0) {
              ...
              msg(
                "You get average cross-validation score of 62.61083743842364%!",
              );
            }
          }
        } else if (phase_d.gate % 13 == 0) {
          if (phase_e.gate % 2 == 0) {
            if (phase_f.gate % 2 == 0) {
              ...
              msg(
                "You get average cross-validation score of 64.67980295566502%!",
              );
            }
          } else if (phase_e.gate % 3 == 0) {
            if (phase_f.gate % 2 == 0) {
            ...
            if (phase_f.gate % 2 == 0) {
              ...
              msg(
                "You get average cross-validation score of 62.58620689655172%!",
              );
            }
          }
        }
        ...
        msg(
          "Yikes, you took the wrong CSP components and got chance-level performance :( Should've picked components from both ends of the eigenvalue spectrum. You can reload the page to try again!",
        );
      }
    } else {
      msg(
        "Yikes, you applied both filters thinking we wouldn't think of that outcome, didn't you? Well now your data is just messed up, so of course you're gonna get chance level performance. What did you expect?",
      );
    }
  },
  dests: [],
